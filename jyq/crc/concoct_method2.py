...
#       print(sample_name) 
        cmd="bowtie2 --no-mixed -f --fr -x contigs/final.contigs.cut.500bp.95%.identity.fa -1  "+"samples/"+sample_name+'/'+ source_1[i]+" -2 "+"samples/"+sample_name+'/'+source_2[i]+" -S "+sample_name+"_pair.sam -p 1"
        print(cmd)
    ...
    if(i<len(source_1)-1):
        ...
        while True:
            ...
            for j in range(i,min(i+finished,len(source_1))):
                ...
                cmd="bowtie2 --no-mixed -f --fr -x contigs/final.contigs.cut.500bp.95%.identity.fa -1  "+"samples/"+sample_name+'/'+ source_1[j]+" -2 "+"samples/"+sample_name+'/'+source_2[j]+" -S "+sample_name+"_pair.sam -p 1"
                print(cmd)
                table.append(subprocess.Popen(cmd,shell=True,env=new_env))
